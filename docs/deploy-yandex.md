...
Когда в `.env` задан `OWNER_CHAT_ID`, владельцу доступна команда `/stats`: сводка
по пользователям и картинка с активностью по дням и часам. Для остальных команды
не существует. Статистика строится из счётчиков «день, час, вид события, число» —
кто именно нажимал, не записывается.

Та же сводка приходит сама каждое утро в `OWNER_STATS_AT` по поясу `OWNER_STATS_TZ`
(по умолчанию 06:00 по Киеву). Пустое `OWNER_STATS_AT` выключает рассылку.

## Отметка посещаемости по QR (только владелец)

Экспериментальная функция: владелец на очной паре шлёт боту фото QR
преподавателя, и бот отмечает присутствие в СДО `lms.ranepa.ru` за него
(механика — в [api-notes.md](api-notes.md#отметка-посещаемости-в-сдо-moodle)).
Включается, только когда в `.env` заданы креды СДО:

```
LMS_LOGIN=            # логин lms.ranepa.ru — тот же, что у кабинета
LMS_PASSWORD=         # пароль там же
```

Работает лишь вместе с `OWNER_CHAT_ID`: фото принимаются только от владельца,
для остальных функции нет. Пусто — функция выключена.

Важно про хранение. В отличие от кабинета, где лежит лишь refresh-токен, здесь
на сервере хранится **сам пароль** СДО — Moodle иначе не пускает. Поэтому пока
это только аккаунт владельца и только через окружение, без общей базы: массовое
(шифрованное) хранение — отдельное решение, если функция станет подпиской.
Декодер QR (`libzbar0` + `pyzbar`) уже входит в образ.
