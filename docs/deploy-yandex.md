...
## JS-проверка кабинета

Российского адреса кабинету мало: с адресов дата-центров он отдаёт вместо
API страницу-проверку антибота, которая исполняется только в браузере
(подробности в [api-notes.md](api-notes.md#js-проверка-антибота)). Поэтому в
образ входит headless Chromium, а в `.env` должно быть:

```
RANEPA_CHALLENGE_SOLVER=playwright
```

Бот проходит проверку настоящим браузером один раз примерно на полчаса и
дальше ходит в API с полученными cookie. В логе это выглядит так:

```
INFO app.ranepa.challenge: Проверка кабинета пройдена, cookie действуют ещё 30 мин
```

Chromium прибавляет к образу около 400 МБ и потребляет 200–300 МБ памяти на
те секунды, пока решает проверку; машине с 1 ГБ хватает. Если бот работает
там, где проверки нет (домашний адрес, сеть академии), можно поставить `none`.

Проверить, показывает ли кабинет проверку с вашего адреса, без бота:

```bash
python3 scripts/probe_exits.py /dev/null
```

Вердикт `НЕЯСНО (200)` с `login-body: <html>…gorizontal-vertikal` — это она.
...
- **«Кабинет требует браузерную проверку»** — в `.env` нет
  `RANEPA_CHALLENGE_SOLVER=playwright`, либо Chromium не смог стартовать:
  смотрите `docker compose logs bot` на строки от `app.ranepa.challenge`.
